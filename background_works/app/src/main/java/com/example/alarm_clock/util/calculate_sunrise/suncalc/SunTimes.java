...
import static com.example.alarm_clock.util.calculate_sunrise.suncalc.util.ExtendedMath.*;
...
import com.example.alarm_clock.util.calculate_sunrise.suncalc.util.QuadraticInterpolation;
import com.example.alarm_clock.util.calculate_sunrise.suncalc.util.Sun;
...
public class SunTimes {
    ...
    public interface Parameters extends
        ...
        default Parameters oneDay() {
            return limit(getDaysDuration(1L)/*Duration.ofDays(1L)*/);
        }
        ...
        default Parameters fullCycle() {
            return limit(getDaysDuration(365L)/*Duration.ofDays(365L)*/);
        }

        public default Duration getDaysDuration(Long count) {
    ...
    private static class SunTimesBuilder extends BaseBuilder<Parameters> implements Parameters {
        ...
        private Duration limit = getDaysDuration(365L);//Duration.ofDays(365L);

        public Duration getDaysDuration(Long count) {
            return Duration.ofDays(365L);
        }
        ...
        public SunTimes execute() {
            ...
            int hour = 0;
            double limitHours = 0;
            limitHours = limit.toMillis() / (60 * 60 * 1000.0);
